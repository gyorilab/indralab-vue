    ...
    <div class="row cchild" style="border-top: 1px solid #ffffff">
      <div class="col" style="padding: 0; border-top: 1px solid #ffffff">
        <div class="form-inline">
          <select class="form-control form-elem" v-model="error_type">
            <option value="" selected disabled hidden>
              Select error type...
            </option>
            <option
              v-for="(option_label, option_value) in options"
              :value="option_value"
              :key="option_value"
            >
              {{ option_label }}
          ...
          />
          <button
            type="button curation_button form-elem"
        ...
        <div
          v-if="
            ...
            error_loading_previous ||
            (no_auth && num_prior_curations !== null && num_prior_curations > 0)
          "
        ...
        >
          <h5>
            ...
            >
              <img
                src="https://bigmech.s3.amazonaws.com/indra-db/reload.png"
          ...
          <div v-if="error_loading_previous">
            <i style="color: red"
              >Sorry, we could not load previous curations:
              {{ error_loading_previous }}</i
            >
          </div>
          <div v-if="previous && previous.length">
            ...
            >
              ...
              <div
                v-for="attr in ['curator', 'tag', 'text', 'source']"
                :key="attr"
                class="col"
              >
...
export default {
  ...
  methods: {
    submitForm() {
      ...
      if (!this.error_type) {
        alert(
          'Please enter an error type or "correct" for the statement in the dropdown menu.'
        );
      ...
      if (this.status === "submitting") this.status = "unknown failure";
    },

    loadPrevious() {
      // todo: load previous curations when user opens the dropdown instead of relying on pre-loading @ server side
    ...
    submitCuration: async function () {
      ...
      if (this.curation_url[this.curation_url.length - 1] !== "/") url += "/";
      url += this.stmt_hash;
      ...
      switch (resp.status) {
        ...
        case 401:
          this.message =
            resp.status + ": You are not authorized, please log in.";
          this.status = "failure";
          break;
        default:
    ...
    getCurations: async function () {
      const resp = await fetch(
        `${this.curation_list_url}/${this.stmt_hash}/${this.source_hash}`,
        {
          method: "GET",
        }
      );
      ...
      switch (resp.status) {
        case 200:
          ...
          for (let entry of data) entry.date = new Date(entry.date);
          this.previous = data;
  ...
  watch: {
    open(newOpen, oldOpen) {
      if (!oldOpen && newOpen && this.previous == null) this.getCurations();
    },
    status(status) {
